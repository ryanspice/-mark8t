<script>
	...
	function getLatestDataFromLocalStorage() {
		...
		layoutSelected =
			JSON.parse(localStorage.getObject("--data-layoutSelected")) ||
			layouts[0];
	}
	...
	const handleWheel = (e) => {
		...
		if (
			t.scrollLeft > 0 &&
			t.scrollLeft < t.scrollWidth - t.clientWidth - 1
		) {
			e.preventDefault();
			e.stopPropagation();
		}
...
<p>
	...
	levels. With our user-friendly interface, managing your product catalog has
	never been easier. Whether you're a small business owner or part of a larger
	team, this page is designed to make your product management tasks simple and
	efficient. Thank you for choosing our platform to manage your products.
