		...
		try {
			if (navigator.share !== undefined) {
				navigator.share({
					...
					text: website.siteName + " " + website.siteAddress + " " + website.siteContactEmail + " " + website.siteContactPhone,
					url: website.siteUrl,
				});
			}
		} catch (e) {
			...
			navigator.clipboard.writeText(website.siteName + " " + website.siteAddress + " " + website.siteContactEmail + " " + website.siteContactPhone);
		};
...
{#if (enabledShare)}
<li>
	<ExternalLink href="#" icon="share" alt="Share Page" clickHandler={handleShareButton}
		style="left:3px;transform: scale(1.3);" label={lblShare} />
</li>
{/if}