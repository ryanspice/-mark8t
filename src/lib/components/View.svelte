<script>
  ...
  function getLatestDataFromLocalStorage() {
    ...
    layoutSelected = JSON.parse(localStorage.getItem('--data-layoutSelected')) || layouts[0];
  }
  ...
  function getPanelInfo() {
    ...
    panelProducts = localStorage.getItem('--panel--panelProducts') === 'true' || false;
    panel1Open = localStorage.getItem('--panel--panel1Open') === 'true' || false;
    ...
    panel4Open = localStorage.getItem('--panel--panel4Open') === 'true' || false;
    panelLayouts = localStorage.getItem('--panel--panelLayouts') === 'true' || false;
  ...
  let scrollPosition;
  $: () => {
    document.getElementById('saved-changes').style = `position:absolute;top: ${scrollPosition}px;`
  };
</script>
<styles />
<style>
...
<div id="saved-changes" class="top-app-bar-container">
  <Banner bind:open bind:centered bind:mobileStacked on:SMUIBanner:closed={handleBannerClosed}>
    ...
    <Label slot="label">There are unsaved changes.
...
<section class="container">
  ...
  <Paper elevation={0}>
    ...
    <br />
    <Modules.Account unsavedChanges={unsavedChanges} />
    <br />
    <!-- WEBSITTE -->
    <Modules.Website unsavedChanges={unsavedChanges} />
    <!-- PRODUCTS -->
    <Accordion>
      <Panel bind:open={panelProducts} on:click={(e)=>{localStorage.setItem('--panel--panelProducts',panelProducts);}}>
        <Header>
          ...
          <Button raised disabled={!panelProducts} on:click={(e)=>{ onAddProduct(); e.preventDefault();
            e.stopPropagation(); }}>Add New</Button>
          <Button raised disabled on:click={(e)=>{ e.preventDefault(); e.stopPropagation(); }}>View All</Button>
          <!-- <Button raised on:click={onAddProduct}>View All</Button> -->
        </Header>
        <Content bind:this={productContainer} style="display:flex;overflow:auto;" on:wheel={handleWheel}>
          ...
          <Card on:click={(e)=> {data = product;console.log(data); openDialogForEditing=true; e.stopPropagation();}}>
            <span style='content: "\003F";opacity:0.01;left:37%;top:40%;font-size:166px;position:absolute;'>
              &#63;</span>
            <img src={product.thumb} style="width:100%;height:100%;object-fit:cover;max-width:275px;" />
            <Content>{product.name}</Content>
            <Actions>
              <ActionButtons>
                <Button on:click={(e)=> {data = product;console.log(data); openDialogForEditing=true;
                  e.stopPropagation();}}>
                  <Label>Edit</Label>
                </Button>
                <Button style="color:red;" on:click={(e)=> {
    ...
    <!-- LAYOUDS -->
    <Accordion>
      <Panel bind:open={panelLayouts} on:click={(e)=>{localStorage.setItem('--panel--panelLayouts',panelLayouts);}}
        disabled>
        <Header>
        ...
        <Content style="display:flex;overflow:scroll;" on:wheel={handleWheel}>
          ...
          <Card on:click={(e)=> {data = layout; layoutSelected = layout; e.stopPropagation();}}>
            <span style='content: "\003F";opacity:0.1;left:37%;top:40%;font-size:166px;position:absolute;'> &#63;</span>
            <img src={layout.image} style="width:100%;height:100%;object-fit:cover;max-width:275px;" />
            <Content>{layout.name}</Content>
            <Actions>
              <ActionButtons>
                <Button on:click={(e)=> {data = layout; layoutSelected = layout; e.stopPropagation();}}>
    ...
    <br />
    <Modules.Google unsavedChanges={unsavedChanges} />
  </Paper>
</section>
<DialogForEditing data={data} open={openDialogForEditing} onSave={onSaveProducts} onClose={()=>{
  openDialogForEditing=false;
  }} onDismiss={()=>
  {
  openDialogForEditing=false;
  ...
  onDelete={(id)=>{ removeProduct(id); unsavedChanges(); }} />