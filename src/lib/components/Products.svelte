<script>
	...
	onMount(() => {
		...
		frame.onload = () => {
			...
			setTimeout(() => {
				// Lazy 
				if (document.getElementById('effects').style.height === document.getElementById('section__products').scrollHeight) {
					return;
				}
				document.getElementById('effects').style.height = document.getElementById('section__products').scrollHeight + "px";
				setTimeout(() => {
					// Lazy 
					if (document.getElementById('effects').style.height === document.getElementById('section__products').scrollHeight) {
						return;
...
<section id="section__products">
	<iframe id="effects" src="" bind:this={frame} frameborder="0" scrolling="no"> </iframe>
	<div>
		<div class='row'>
			<div class='column'>
				<div>
